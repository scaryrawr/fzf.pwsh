        ...
        if (Test-Path -Path 'package.json')
        {
            ...
            if (Get-Content -Path 'package.json' -Raw | Select-String -Pattern '"workspaces"')
            {
                ...
                if (Get-Command 'yarn' -ErrorAction SilentlyContinue)
                {
                    try
                    {
                        # Check if yarn is version 1.x
                        $yarnVersion = yarn --version 2>$null
                        $isYarnV1 = $yarnVersion -and $yarnVersion -match '^1\.'
                        ...
                        {
                            $packagesInfo = yarn workspaces list --json 2>$null | ConvertFrom-Json | Where-Object { $_.location -ne '.' } | ForEach-Object {
                                [PSCustomObject]@{
                                    name = $_.name
                                    path = "./$($_.location)/package.json"
                                }
                            }
                            $packagesInfo | ConvertTo-Json | Set-Content -Path $cacheFile
                        }
